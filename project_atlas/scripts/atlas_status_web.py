...
function drawRadarHubWaiting(status){
 for(const id of ['radarScope','radarTwin']){let c=$(id),g=c.getContext('2d');g.fillStyle='rgba(2,8,12,.88)';g.fillRect(0,c.height/2-34,c.width,68);g.fillStyle='#17d5ff';g.font='bold 17px system-ui';g.textAlign='center';g.fillText('MEGA HUB ONLINE — WAITING FOR RADAR UART',c.width/2,c.height/2-4);g.fillStyle='#9bb9ca';g.font='11px system-ui';g.fillText(status,c.width/2,c.height/2+18)}
 $('radarCaption').textContent=status+' • check RD-03D power/GND and Mega Serial2: radar TX → RX2 pin 17, radar RX → TX2 pin 16';
}
async function pollRadar(){try{let d=await fetch('/api/radar',{cache:'no-store'}).then(x=>x.json()),r=d.radar||{},raw=val(r,'radar',''),live=!!r.radar&&r.radar.age<1,linkFresh=!!r.radar_link&&r.radar_link.age<2.5;lastRadarLink=linkFresh?String(val(r,'radar_link','')):'';lastRadarTargets=parseRadarTargets(raw);lastRadarLive=live;drawRadar(raw,live);if(radarView==='3d')drawRadarTwins(lastRadarTargets,live);if(!live&&linkFresh)drawRadarHubWaiting(lastRadarLink)}catch(e){lastRadarTargets=[];lastRadarLive=false;lastRadarLink='';drawRadar('',false);if(radarView==='3d')drawRadarTwins([],false)}}setInterval(pollRadar,125);pollRadar();
